...
fun projectCreationRoute(projects: Projects, htmlView: BiDiBodyLens<ViewModel>) = routes(
    "/" bind Method.GET to showNewProjectForm(htmlView),
    "/" bind Method.POST to addProject(projects, htmlView)
)

fun showNewProjectForm(htmlView: BiDiBodyLens<ViewModel>): HttpHandler = {
    Response(OK).with(htmlView of NewProjectDataViewModel())
}
...
fun addProject(projects: Projects, htmlView: BiDiBodyLens<ViewModel>): HttpHandler = { request ->
    ...
    } else Response(OK).with(htmlView of NewProjectDataViewModel(webForm))
}
